...
import com.kmpile.lingua.internal.Constant.CHARS_TO_LANGUAGES_MAPPING
import com.kmpile.lingua.internal.Constant.MULTIPLE_WHITESPACE
import com.kmpile.lingua.internal.Constant.NO_LETTER
import com.kmpile.lingua.internal.Constant.NUMBERS
import com.kmpile.lingua.internal.Constant.PUNCTUATION
import com.kmpile.lingua.internal.Constant.isJapaneseAlphabet
import com.kmpile.lingua.internal.Ngram
import com.kmpile.lingua.internal.TestDataLanguageModel
import com.kmpile.lingua.internal.TrainingDataLanguageModel
import com.kmpile.lingua.internal.readLanguageModelJson
import com.kmpile.lingua.internal.runTasksInParallel
import com.kmpile.lingua.internal.util.extension.enumMapOf
import com.kmpile.lingua.internal.util.extension.incrementCounter
import com.kmpile.lingua.internal.util.extension.isLogogram
...
class LanguageDetector internal constructor(
    ...
    fun computeLanguageConfidenceValues(text: String): Map<Language, Double> {
        val values = LinkedHashMap<Language, Double>()
        ...
        val allProbabilitiesAndUnigramCounts = runTasksInParallel(tasks)
        ...
        val highestProbability = summedUpProbabilities.maxByOrNull { it.value }?.value ?: return emptyMap()
        ...
        return confidenceValues.keys
            .sortedWith(sortedByConfidenceValueThenByLanguage)
            .associateWith { confidenceValues.getValue(it) }
    }
    ...
    internal fun lookUpNgramProbability(
        ...
        return model[ngram.value] ?: 0F
    }

    private fun preloadLanguageModels() {
        val tasks = mutableListOf<() -> Map<String, Float>>()

        for (language in languages) {
            tasks.add { loadLanguageModels(trigramLanguageModels, language, 3) }

            if (!isLowAccuracyModeEnabled) {
                tasks.add { loadLanguageModels(unigramLanguageModels, language, 1) }
                tasks.add { loadLanguageModels(bigramLanguageModels, language, 2) }
                tasks.add { loadLanguageModels(quadrigramLanguageModels, language, 4) }
                tasks.add { loadLanguageModels(fivegramLanguageModels, language, 5) }
            }
        }
    ...
    internal companion object {
        ...
        private val modelsLock = SynchronizedObject()

        internal val unigramLanguageModels = enumMapOf<Language, Map<String, Float>>()
        internal val bigramLanguageModels = enumMapOf<Language, Map<String, Float>>()
        internal val trigramLanguageModels = enumMapOf<Language, Map<String, Float>>()
        internal val quadrigramLanguageModels = enumMapOf<Language, Map<String, Float>>()
        internal val fivegramLanguageModels = enumMapOf<Language, Map<String, Float>>()

        private fun loadLanguageModels(
            languageModels: MutableMap<Language, Map<String, Float>>,
            language: Language,
            ngramLength: Int,
        ): Map<String, Float> {
            synchronized(modelsLock) {
                languageModels[language]?.let { return it }
            }
            val model = loadLanguageModel(language, ngramLength)
            synchronized(modelsLock) {
                return languageModels.getOrPut(language) { model }
            }
        }
        ...
        private fun loadLanguageModel(
            ...
            val json = readLanguageModelJson(filePath) ?: return emptyMap()
            return TrainingDataLanguageModel.fromJson(json)
        }
    }
}
